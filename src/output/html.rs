            ...
            Event::Enter(Container::SpecialBlock(block)) => {
                    ...
                    && let Some(name) = par.nth(1)
                {
                        ...
                        && let Some(usr) = par.next()
                    {
                        ...
                        if !usr.is_empty() {
                            let desc = if let Some((person, expression)) = usr.rsplit_once('/') {
                                format!("{person} is {expression}")
                            } else {
                                usr.to_string()
                            };
                            ...
                            self.exp.push_str(format!(
                                r#"faces/{}.png" alt="{} says" title="{1}"><div class=chat-text>"#,
                                slugify!(usr),
                                HtmlEscape(desc)
                            ));
...
** DONE finish writing this test",
        ...
        assert_eq!(
            exp.exp.finish(),
            r##"<main><section></section><h2 tabindex=-1 id="meow">meow <a class=see-focus href="#meow" aria-label="permalink to section">§</a></h2><section><div class="chat"><img class=chat-head width=64 src="faces/fox.png" alt="fox says" title="fox"><div class=chat-text><p>AAAA
</p></div></div><p>i have a footnote<sup><a id="fnr.1.0" href="#fn.1" role=doc-noteref>[1]</a></sup><sup><a id="fnr.2.0" href="#fn.2" role=doc-noteref>[2]</a></sup>
